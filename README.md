...
| Zero-leak profiles | 支持 `balanced`、`private`、`strict` 安全策略；private/strict 会阻止高风险文件正文输出。 |
...
| `--profile balanced|private|strict` | 选择安全策略，默认 `balanced`。 |
...
- `audit`：扫描、忽略、脱敏统计。
- `audit.securityPolicy`、`audit.riskScore`、`audit.blockedHighRiskFiles`：实际使用的安全策略和风险统计。
...
- `balanced`：默认策略。保留有用上下文，并脱敏常见 secret。
- `private`：私仓策略。高风险文件即使被显式 include，也只输出阻止摘要，不输出正文。
- `strict`：最大保护策略。高风险文件正文被阻止，并在 audit 中记录 blocked high-risk files 与 risk score。
...
CtxSift alpha benchmark 使用 6 个本地 fixture 覆盖 simple TypeScript、pnpm monorepo、Turbo/Nx metadata、strict security profile 和 PR diff review bundle。基准不调用 LLM、不需要 API key，secret fixture 只使用假值。

当前提交的 fixture 报告：

- 平均 relevant hit rate：83.3%。
- 平均 token saving ratio：39.8%。
- 平均 workspace package hit rate：33.3%。
...
完整指标见 [Benchmark Toolkit](docs/benchmark.md)、[benchmark-report.md](benchmark-report.md) 和 [benchmark-report.json](benchmark-report.json)。
